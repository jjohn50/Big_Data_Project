...
from pyspark.ml.classification import GBTClassifier  #classification models
...
sort_preds_gbt = preds_gbt.select('userID', 'trackID', 'label', 'probability', 'rawPrediction', 'prediction').sort(col('userID').asc(), col('probability').desc())
sort_preds_gbt.show(6)

gbt_preds = gbt_model.transform(prediction_df)    # transform prediction_df with gradient-boosted tree model
gbt_preds.select('userID', 'trackID', 'probability', 'rawPrediction', 'prediction').show(12)

sort_gbt_pres = gbt_predictions.select('userID', 'trackID', 'probability', 'rawPrediction', 'prediction').sort(col('userID').asc(), col('probability').desc())
sort_gbt_pres.show(6)

pd_sort_gbt_preds = sort_gbt_pres.toPandas().fillna(0.0)    # create pandas df

pd_sort_gbt_preds

columns_to_write = ['userID', 'trackID']
pd_sort_gbt_preds.to_csv('gbt_predictions.csv', index=False, header=None, columns=columns_to_write)

f_gbt_preds = open('gbt_predictions.csv')   
f_gbt_final_preds = open('gbt_final_predictions.csv', 'w')

f_gbt_final_preds.write('TrackID,Predictor\n')

user_id = -1
track_id = [0] * 6

# Go through each line of the predictions file
for line in f_gbt_preds:
    ...
    if i == 6:                               # if last entry for current user reached
        ...
        for ii in range(0, 6):         
            ...
            f_gbt_final_preds.write(out_str + '\n')

f_gbt_preds.close() 
f_gbt_final_preds.close()   