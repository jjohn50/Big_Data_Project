...
from pyspark.ml.classification import RandomForestClassifier  #classification models
...
ranfor_model = ranfor.fit(train_df)

preds_rf = ranfor_model.transform(test_df)

sort_preds_rf = preds_rf.select('userID', 'trackID', 'label', 'probability', 'rawPrediction', 'prediction').sort(col('userID').asc(), col('probability').desc())
sort_preds_rf.show(6)  

rf_preds = ranfor_model.transform(prediction_df)    # transform prediction_df with random forest model
rf_preds.select('userID', 'trackID', 'probability', 'rawPrediction', 'prediction').show(12)

sort_rf_preds = rf_preds.select('userID', 'trackID', 'probability', 'rawPrediction', 'prediction').sort(col('userID').asc(), col('probability').desc())
sort_rf_preds.show(6)

pd_sort_rf_preds = sort_rf_preds.toPandas().fillna(0.0)    # create pandas df

pd_sort_rf_preds

columns_to_write = ['userID', 'trackID']
pd_sort_rf_preds.to_csv('rf_predictions.csv', index=False, header=None, columns=columns_to_write)

f_ranfor_preds = open('rf_predictions.csv')  
f_ranfor_final_preds = open('rf_final_predictions.csv', 'w')

# Write header
f_ranfor_final_preds.write('TrackID,Predictor\n')

user_id = -1
track_id = [0] * 6

# Go through each line of the predictions file
for line in f_ranfor_preds:
    ...
    if i == 6:                               # if last entry for current user reached
        ...
        for ii in range(0, 6):         
            ...
            f_ranfor_final_preds.write(out_str + '\n')

f_ranfor_preds.close()         
f_ranfor_final_preds.close()  